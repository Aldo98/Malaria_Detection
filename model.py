...
class CNN(nn.Module):
  def __init__(self, args, num_classes=1000):
    ...
    if args.net_type == '1graph':
      self.conv2 = Triplet_unit(args.channels // 2, args.channels, 2)
      self.conv3 = Triplet_unit(args.channels, args.channels, 2)
      if args.resume:
      ...
      self.conv5 = Triplet_unit(args.channels *2, args.channels*4, 2)
      self.relu = nn.ReLU()
      self.conv = nn.Conv2d(args.channels * 4, 1280, kernel_size=1)
      self.bn2 = nn.BatchNorm2d(1280)
    if args.net_type == '2graph':
      self.conv2 = Triplet_unit(args.channels // 2, args.channels, 2)
      self.conv3 = Triplet_unit(args.channels, args.channels, 2)
      if args.resume:
        graph = load_graph(os.path.join(args.model_dir, 'conv4.yaml'))
